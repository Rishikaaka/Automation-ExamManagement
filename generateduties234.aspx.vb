...
Partial Class generateduties234
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Literal1.Text = "<h1> free rooms</h1>"
        While dr1.Read()
        ...
        Literal1.Text = Literal1.Text & "<h2>Available students=" & studroomallot.Count & "</h2>"

        For Each stud In studroomallot
            Literal1.Text = Literal1.Text & "<br>" & stud
        Next

        Literal1.Text = Literal1.Text & "<h2>Left over students<h2><br>"
        ...
        For Each freeroom In freerooms
            ...
            For i = 1 To Integer.Parse(roomsplit(3))
                If studroomallot.Count > 0 Then
                    Dim stud() As String = Split(studroomallot(0), ",")
                    Dim stud1() As String = stud(0).Split("-")
                    sids = sids & stud1(0) & ","
                    Dim stud2() As String = stud(1).Split("-")
                    sids = sids & stud2(0) & ","
                    studroomallot.RemoveAt(0)
                    If studroomallot.Count = 0 Then
                        completed = True
                        Exit For
                    End If
            ...
            If completed = True Then
                ...
                If i < Integer.Parse(roomsplit(3)) Then  'room have space for leftover studs

                    For j = i To Integer.Parse(roomsplit(3))
                        Dim stud() As String = Split(leftoverstuds(0), "-")
                        sids = sids & stud(0) & ","
                        leftoverstuds.RemoveAt(0)
                        If leftoverstuds.Count = 0 Then
                            Exit For
                        End If
                    Next
                End If
        ...
        Literal1.Text = Literal1.Text & "<h1>Studs alloted to rooms..</h1>"
        ...
        For Each examday In examtimetable
            ...
            For Each faculty In freefaculty
                ...
                ''''''''
                ''''''''
                Dim con6 As SqlConnection = DataBaseConnection.getDBConnection()
                con6.Open()
                Dim qry6 = "select count(*) nduties from duties where fid='" & faculty.Split("-")(0) & "'"

                Dim com6 As New SqlCommand(qry6, con6)
                Dim dr6 As SqlDataReader = com6.ExecuteReader()
                If dr6.Read Then
                    If dr6("nduties") > 3 Then
                        ' Literal1.Text = Literal1.Text & " faculty " & faculty.Split("-")(0) & " completed his duties"
                        Continue For
                    End If
                End If
